...
export async function getCarteraMora(): Promise<CarteraMoraData> {
  ...
  const empresaNombre = settings?.companyName ?? 'LMS Credit Core';
  const empresaDireccion = settings?.companyAddress ?? 'Santo Domingo, República Dominicana';
  const empresaTelefono = settings?.companyPhone ?? '';
  const empresaRnc = settings?.companyRnc ?? '';
  ...
  const prestamos: CarteraMoraLoan[] = loans.map((loan) => {
    ...
    // Monto mora: suma de late fees registradas en pagos + estimado pendiente
    const montoMoraRegistrado = loan.payments.reduce(
      (sum, p) => sum + Number(p.lateFee ?? 0),
      0
    );
